    ...
    def read_value(self):
        ...
        if self.last_value is None:  # Pierwsze przypisanie wartośći temperatury
            ...
            value = max(self.min_value,
                        min(self.max_value, value))  # # Sprawdzenie czy nowa wartość mieści się w zalooznych granicach
        ...
        for callback in self.callbacks:  # Generowanie callback'ów dla tego sensora
            try:
                callback(self.sensor_id, timestamp, self.last_value,
                         self.unit)  # Callback pobiera dane z czujnika i przekazuje go do funkcji register_callback
            except Exception as e:
                print(
                    f"Blad w callbacku {callback.__name__} dla {self.sensor_id}: {str(e)}")  # W przypadku błędu zwraca tresc bledu
        ...
        if self.base_humidity is None:  # Pierwsze losowanie bazowej wartości wilgotności
            ...
            if self.temperature_sensor and self.temperature_sensor.last_value is not None:
                ...
                if (temp > 15):  # Spadek wilgotności, gdy temperatura jest większa niż 15 stopni
                    zmiennosc = (
                                            temp - 15) * -1  # Za każdy spodniej różnicy temperatury, wilgotność spada o punkt procentowy
                else:
                    zmiennosc = (
                                            15 - temp) * 1  # Za każdy spodniej różnicy temperatury, wilgotność wzrasta o punkt procentowy

                value = self.base_humidity * (
                            1 + zmiennosc / 100)  # Obliczanie nowej wartości po uiszczeniu procentowym
...
class PressureSensor(Sensor):
    def __init__(self, sensor_id, frequency=2):
        ...
        self.base_value = np.random.uniform(self.min_value + 5,
                                            self.max_value - 5)  # Pierotne solowanie wartości ciśnienia
